...
`include "sm-TestRandDelayMem_1port.v"
...
#(
  ...
  //----------------------------------------------------------------------
  // spread src_msg out to individual RoCC pins
  logic [`ASIC_SRC_MSG_NBITS-1:0] cmd_rs1_i;
  logic [`ASIC_SRC_MSG_NBITS-1:0] cmd_rs2_i;
  logic [6:0]  cmd_inst_funct_i;
  logic [6:0]  cmd_inst_opcode_i;

  sm_RoccCmdUnpack#(`ASIC_SRC_MSG_NBITS) src_msg_unpack
  (
    .msg             (src_msg),           // src_msg ->|
    .cmd_rs2         (cmd_rs2_i),         //           |-> cmd_rs2_i
  ...
  //----------------------------------------------------------------------
  logic [4:0]  resp_rd_o;
  logic [`ASIC_SNK_MSG_NBITS-1:0] resp_data_o;

  sm_RoccCmdPack#(`ASIC_SNK_MSG_NBITS) snk_msg_pack
  (
...
module top;
  ...
  rv64_InstTasks rv64(); // see lib/proc/rv-inst.v
  ...
  //----------------------------------------------------------------------
  // load_rocc_i_i_i: helper task to load an entry into the from_mngr source
  //----------------------------------------------------------------------
  ...
  (
    input logic [9:0]                           i,
    input logic [2*`ASIC_SRC_MSG_NBITS+32-1:0]  msg
  );
  begin
    th.src.src.m[i] = msg;
  end
  ...
  //----------------------------------------------------------------------
  // src msg will be { rs1 data , assembled cmd inst }
  task inst_src
  (
  ...
  begin
    th_inst_asm_str = rv64.asm( th_src_idx, asm_str );       // pc (not used), assembly instr string -> binary instr
    load_from_mngr( th_src_idx, {rs2,rs1,th_inst_asm_str} ); // load src with { rs2 data, rs1 data, assembled cmd instr }
    th_src_idx = th_src_idx + 1;
    // we set the next address with x's so that src/sink stops here if
    // there isn't another call to init_src/sink
    // load_from_mngr( th_src_idx, 64'hxxxx_xxxx_xxxx_xxxx );
    load_from_mngr( th_src_idx, 160'hx );
  end
  endtask
